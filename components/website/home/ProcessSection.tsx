...
export default function ProcessSection({ miniTitle, steps, image }: ProcessSectionProps) {
        ...
        <div className="flex flex-col lg:flex-row gap-10 lg:gap-20">
          ...
          <div className="hidden lg:block lg:w-1/2">
            <div className="sticky top-28 aspect-[4/5] rounded-sm overflow-hidden bg-[var(--ds-bg)] relative">
              {/* If an 'image' prop is passed, use it, otherwise show step-specific images */}
              {image ? (
                <img 
                  src={image} 
                  alt="Process" 
                ...
                items.map((step, i) => (
                  <motion.img
                    key={step.id || i}
                    src={resolveMediaUrl(step.associated_image) || "/How.webp"}
                    alt={step.title}
